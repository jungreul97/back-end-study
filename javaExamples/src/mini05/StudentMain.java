package mini05;
// 전체 프로젝트에서 검색하기
// CRTL + SHILFT + F
import DBMS.MemberVO;

import java.util.ArrayList;
import java.util.List;
import java.util.Scanner;

//성적관리 프로그램
//클래스 설계
class Student {
    String name;
    int korean;
    int english;
    int math;
    //필드가 있는 생성자

    public Student(String name, int korean, int english, int math) {
        this.name = name;
        this.korean = korean;
        this.english = english;
        this.math = math;
    }
}
public class StudentMain {
    public static void main(String[] args) {

        //ArrayList 관련 함수들 : 전체지우기, 추가, 삭제, 검색(직접 코드로)
        //5. 성적 관리 프로그램
        // ArrayList 클래스 객체 배열을 사용해보자.
        //입력 및 출력 예시
        //-----------성적 관리 프로그램-------------
        //1.입력 2.전체출력 3.검색 4.수정 5.삭제 6.종료 : 1
        //이름 입력 : 홍길동
        //국어점수 입력 : 70
        //영어점수 입력 : 80
        //수학점수 입력 : 90
        //-----------성적 관리 프로그램-------------
        //1.입력 2.전체출력 3.검색 4.수정 5.삭제 6.종료 : 2
        //이름: 홍길동 국어: 70 영어: 80 수학: 90 총점: 240 평균: 80.0
        StudentDAO dao = new StudentDAO();
        Scanner scanner = new Scanner(System.in);
        int studentn = 2;
        while(true){
            System.out.println();
            System.out.println("-----성적 관리 프로그램-----");
            System.out.print("1.입력 2.전체출력 3.검색 4.수정 5.삭제 6.종료 : ");
            int menu = scanner.nextInt();
            List<StudentVO> list = dao.list();
            if(menu == 1){ //입력
                StudentVO vo = new StudentVO();
                System.out.print("이름 입력:");
                String name = scanner.next();
                System.out.print("국어점수 :");
                int korean = scanner.nextInt();
                System.out.print("영어점수 :");
                int english = scanner.nextInt();
                System.out.print("수학점수 :");
                int math = scanner.nextInt();
                vo.setStudentno(studentn++);
                vo.setName(name);
                vo.setKorean(korean);
                vo.setEnglish(english);
                vo.setMath(math);
                int result = dao.insert(vo);
                if (result > 0) System.out.println("학생 등록 성공 ! ");
                else System.out.println("학생 등록 실패 ! ");
            }else if(menu == 2){ //출력
                //이름: 홍길동 국어: 70 영어: 80 수학: 90 총점: 240 평균: 80.0
                for (StudentVO  student : list){
                    System.out.print("학생번호 : "+student.getStudentno());
                    System.out.print(" 학생이름 : "+student.getName());
                    System.out.print(" 국어 성적 : "+student.getKorean());
                    System.out.print(" 영어 성적 : "+student.getEnglish());
                    System.out.println(" 수학 성적 : "+student.getMath());
                }
            }else if(menu == 3){ //검색
                System.out.print("검색할 사람의 이름 :");
                String searchWord = scanner.next();
                StudentVO student = dao.selectOne(searchWord);

                System.out.print("학생번호 : "+student.getStudentno());
                System.out.print(" 학생이름 : "+student.getName());
                System.out.print(" 국어 성적 : "+student.getKorean());
                System.out.print(" 영어 성적 : "+student.getEnglish());
                System.out.println(" 수학 성적 : "+student.getMath());

            }else if(menu == 4){ //수정
                System.out.print("수정할 사람의 이름 :");
                String name = scanner.next();
                System.out.println();
                System.out.print("수정할 내용은? 1.국어 2.영어 3.수학 :");
                int no = scanner.nextInt();
                System.out.println();
                System.out.print("수정할 점수:");
                int score = scanner.nextInt();
                //검색
                StudentVO student = dao.selectOne(name);
                if(no == 1){
                    student.setKorean(score);
                }
                else if(no == 2){
                    student.setEnglish(score);
                }else{
                    student.setMath(score);
                }
                int result = dao.update(student);
                if (result > 0) System.out.println("수정 성공 !");
                else System.out.println("수정 실패 ! ");

            }else if(menu == 5){ //삭제
                System.out.print("삭제할 사람의 이름 :");
                String name = scanner.next();
                //검색
                String searchWord = name;
                int result = dao.delete(searchWord);
                if(result > 0){
                    System.out.println("삭제 성공");
                }else{
                    System.out.println("삭제 실패");
                }
                studentn--;
            }else if(menu == 6){ //종료
                System.out.println("종료!!");
                break;
            }
        }
        scanner.close();
    }
}
